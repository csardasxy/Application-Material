...
public class TP_Camera : MonoBehaviour {
    ...
    public float NearCameraFarClipPlane = 20f;
    ...
    private float mouseRotate = 1f;
    private float velX = 1f;
    private float velY = 1f;
    private float velZ = 1f;
    private float velDistance = 1f;
    private float velTargetLookAt = 1f;
    private float startDistance = 10f;
    private Vector3 position = Vector3.zero;
    private Vector3 desiredPosition = Vector3.zero;
    private float desiredDistance = 1f;
    private float preOccludedDistance = 1f;
    private float distanceSmooth = 1f;
    private bool isFPS;
	...
	void Start () {
        ...
        target = GameObject.FindGameObjectWithTag("LookingAt").transform;
    ...
    void HandlePlayerInput()
    {
        ...
        {
            mouseX += (Input.GetAxis("Mouse X") + mouseRotate) * MouseSensitivity;
        ...
        {
            desiredDistance = Mathf.Clamp(desiredDistance - Input.GetAxis("Mouse ScrollWheel") * MouseWheelSensitivity, DistanceMin, DistanceMax);
    ...
    void UpdateTargetLookAtPosition()
    {
        ...
        TargetLookAt.transform.position = new Vector3(target.position.x, target.position.y + currentTargetLookAtOffset, target.position.z);
    }
    ...
    void FadePlayer()
    {
        ...
        if (Distance < FadeDistance)
        {
            alpha = (Distance / FadeDistance) - FadeStart;
    ...
    public static void AttachCamera()
    {
        ...
        // Camera
        tmpCamera = GameObject.FindGameObjectWithTag("MainCamera");
        if (tmpCamera == null)
        {
