local _M = class("VIPPanel", lc.ExtendUIWidget)
...
function _M:init(bgName)
    ...
    local title = V.createCheckinTitle(Str(STR.VIP_CHECKIN_TITLE), 0)
    lc.addChildToPos(self, title, cc.p(lc.w(self) / 2 + 140, lc.h(self) - lc.h(title) / 2 - 24))

    local tip = V.createTTF(Str(STR.VIP_CHECKIN_TIP), V.FontSize.S1, V.COLOR_TEXT_DARK)    
    ...
    local bonuses, claimableBonuses, unclaimBonuses, claimedBonuses = {}, P._playerBonus.splitBonus(P._playerBonus._bonusVipDaily, function(i, bonus) return {_bonus = bonus, _label = string.format("VIP %d", i)} end)
