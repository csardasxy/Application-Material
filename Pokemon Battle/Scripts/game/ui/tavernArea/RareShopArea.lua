...
local _M = class("RareShopArea", lc.ExtendCCNode)
...
function _M:generateCardPackageData()
    ...
    for _,v in ipairs(productInfos) do
        ...
        if v._resType == Data.PropsId.rare_coin and P._vip == 0 then show = false end
        if show then
            local cost = v._cost
            table.insert(packages[type], {_infoId=id , _price=cost, _priceType=v._resType,_disCount=j,  _id=v._id})
            local hasBought = false
            if P._playerMarket._rareGoodsMap[type] then hasBought = true end
...
function _M:createRecruitItem(info)
    ...
    local titleBg = V.createRareCardPackage(info)
...
function _M:showCardBox(info)
    ...
    resPanel:setMode(info._type == 0 and Data.PropsId.magic_dust or Data.PropsId.rare_silver_coin)
...
function _M:addCardList(node, packageType)
    ...
    local onBuy = function(sender)
        ...
        ClientData.sendBuyRare(card._id)
    ...
    local confirmBuy = function(sender)
        local card = sender._card
        if P._playerMarket._rareGoodsMap[card._id] then
        ...
        if card._price > P:getItemCount(card._priceType) then
            ToastManager.push(string.format(Str(STR.NOT_ENOUGH), Str(Data._propsInfo[card._priceType]._nameSid)))
        else
            require("Dialog").showDialog(Str(STR.SURE_TO_BUY), function() onBuy(sender) end )
        end
    end
    cardList:registerTapBtnCustom1(confirmBuy)
    cardList:setMode(CardList.ModeType.rare_shop)
    cardList._rareInfo = self._packages[packageType]
    cardList:registerCardSelectedHandler(function (card, index)
        local CardInfoPanel = require("CardInfoPanel")
        local cardInfoPanel = CardInfoPanel.create(card, 1, CardInfoPanel.OperateType.view, cardList._rareInfo[index])
        cardInfoPanel:show()
    end)
